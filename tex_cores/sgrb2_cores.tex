...
to a threshold of 0.5 mJy/beam across the image.  We performed this same
...
The total stellar mass implied is $M_{tot} = 8\ee{3}$ \msun; see Table
\ref{tab:clustermassestimates}.
% If instead we assume
% each source has a mass equal to the
% mean stellar mass for $M>8$ \msun, $\bar{M}=21.1$ \msun, then the total
% inferred mass is $M_{tot}=2\ee{4}$
% \msun.
These are lower limits in the Sgr B2 N and M regions because our
