...
regions M, N, and S.  While all protostars reside in regions of high column
density, not all regions of high column density contain a high density of
...
includes a red dot at the position of each identified continuum pointlike
...
{Images of the 3 mm continuum in the Sgr B2 Deep South (DS) region.  The right figure
includes a red dot at the position of each identified continuum pointlike
...
in prep).  We performed these fits at 25\arcsec resolution, using the 70, 160,
250, and 350 \um data and excluding the 500 \um channel.
...
column density distribution without careful attention to the large implied
uncertainties.  The data combination is discussed in more detail in Appendix
...
background (labeled `conservative' and `aggressive' in Figure
\ref{fig:fluxhist}).  Both of these selection criteria are significantly more
conservative than a local $5-\sigma$ threshold.
...
background noise and make fainter sources difficult to detect, as described
in Section ***.  Additionally, they both contain extended structure...

For a subset of the sources, primarily the brightest, we measured the spectral
index $\alpha$ based on CASA \texttt{tclean}'s fit to the 2-term Taylor
expansion model of the data.  \texttt{tclean} produces $\alpha$ and
$\sigma(\alpha)$ (error on $\alpha$) maps, and we used the $\alpha$ value at
the position of peak intensity for each source.  We include in the catalog only
those sources with $|\alpha| > 5 \sigma(\alpha)$ or $\sigma(\alpha) < 0.1$; the
latter include sources with $\alpha\sim0$ measured at relatively high
precision.  Several of the brightest sources did not have significant
...
\citep{Gaume1995a,Mehringer1995b,de-Pree1996a,de-Pree2015a}, which includes 78
sources.  We classified sources as \hii regions if there is a corresponding 0.7
or 1.3 cm source within one ALMA beam (0.5\arcsec).  The majority of these
are unresolved
...
{A histogram of the flux density of the observed sources.  The two brightest
regions not identified as \hii regions are Sgr B2M f1 and Sgr B2N K2; in these
regions, it is unclear whether the free-free or dust emission dominates.}
...
surrounding them should match of the gas (since we are assuming the gas and
...
As noted at the top of this section, a cold `core' at our detection limit would
have $M(20\mathrm{K})\approx20$ \msun.  At these high densities, it is unlikely
any such cores are unbound.  If we assume higher dust temperatures, the
inferred gas mass is lower, but an internal heating source - i.e., a protostar
- is required.
...
Using a \citet{Kroupa2001a} mass function with
