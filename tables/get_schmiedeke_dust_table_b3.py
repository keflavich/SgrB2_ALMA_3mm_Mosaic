...
rslt = requests.get('http://www.aanda.org/articles/aa/full_html/2016/04/aa27311-15/T6.html')
...
tbl.write(paths.tpath("Schmiedeke2016_dust_tableB3.txt"),
          format='ascii.fixed_width', overwrite=True)


reglist = [regions.EllipseSkyRegion(coordinates.SkyCoord(row['RA'], row['Dec'], frame='fk5', unit=(u.hour, u.deg)),
                                    width=(u.Quantity(float(row['r0,x'])*1000, u.au) / (8.5*u.kpc)).to(u.arcsec, u.dimensionless_angles()),
                                    height=(u.Quantity(float(row['r0,y'])*1000, u.au) / (8.5*u.kpc)).to(u.arcsec, u.dimensionless_angles()),
                                    meta={'text': row['ID']},
                                    visual={'name': row['ID']},
                                   )
           for row in tbl
...
regions.write_ds9(reglist, paths.rpath('Schmiedeke2016_dust_tableB3.reg'))
