...
pairs = [('HCOp',  'CMZ_3mm_HCO+.fits', 'SgrB2_a_03_7M.HCOp.image.pbcor.fits', 89188526000.0),
         ...
         ('H42a',  'CMZ_3mm_RRL_H42a.fits', 'SgrB2_a_03_7M.lowres.spw0.image.pbcor.fits', 92034434000.0),]
...
for species,mopra,aca,restfrq in pairs:
    ...
    h1 = acacube.header
    jyk = (1*u.Jy).to(u.K, u.brightness_temperature(h1['BMAJ']*u.deg * h1['BMIN']*u.deg * 2 * np.pi, h1['RESTFRQ']*u.Hz))

    combcube = fourier_combine_cubes(acac, mc, highresscalefactor=jyk.value, lowresfwhm=0.65*u.arcmin)
    f1 = fits.open(aca)
    f1[0].data = combcube
