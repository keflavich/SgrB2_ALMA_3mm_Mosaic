...
for globstr,zranges in ((("*{0}*image.pbcor.fits"),         ((-3,35),(-5,60),(-5,40),(-5,20))),
    ...
    for (vmin,vmax),(zmin,zmax) in zip(((30,51),(52,93),(94,134),(55,66)),
                                  zranges):
        rgbfn = [glob.glob(pdir(globstr.format(mol).replace(".fits",".v{0}to{1}.mom0.fits".format(vmin,vmax))))[0]
        ...
        if 'Regridded' in globstr:
            ...
            txt = ''
        out = pngdir(os.path.join(os.path.split(globstr)[0],"rgb_hcn_hnc_hcop.{2}v{0}to{1}.mom0.png".format(vmin,vmax, txt)))
        print rgbfn,out
