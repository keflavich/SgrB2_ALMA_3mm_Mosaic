...
time per channel on lustre:
    19:05:07 - 18:51:59 = 14 minutes
...
for spwnum in spwlist:
    ...
    if not os.path.exists(concatvis):
        ...
        cvelvises = ([os.path.join(rootpath,
                     ...
                     for ss in spws['7m'][spwnum].split(",")] +
                     [os.path.join(rootpath,
                                   'SgrB2_a_03_TC.spw{0}.cvel'.format(ss))
                     for ss in spws['TC'][spwnum].split(",")] +
                     [os.path.join(rootpath,
    ...
    for nchan in (1,5,20,40,80):
        for vises in ['concat', 'individual']:
            # include a 1-pixel buffer
            output = 'speedtest_nchan{0}_{1}'.format(nchan, vises)
            ...
            if (not (os.path.exists(output+".image.fits") or
                ...
                tclean(vis = cvelvises if (vises == 'individual') else concatvis,
                       imagename = output,
                       field = '',
