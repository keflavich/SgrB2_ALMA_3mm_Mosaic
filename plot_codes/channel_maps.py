...
for cubefn, suffix, (mn,mx), species in (
    ...
    for ii,(v1,v2) in enumerate(slabs):
        ...
        ax.annotate("${0:d} < v < {1:d}$"
                    .format(int(v1.value), int(v2.value)), (0.1, 0.8),
    ...
    pl.subplots_adjust(hspace=0,
                       wspace=0)
    pl.savefig(paths.fpath('channelmaps/{0}_{1}_channelmaps{2}.png'
                           .format(sourcename,species,suffix)),
               bbox_inches='tight', dpi=150)
    pl.savefig(paths.fpath('channelmaps/{0}_{1}_channelmaps{2}.pdf'
                           .format(sourcename,species,suffix)),
